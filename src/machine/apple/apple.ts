...
export class AppleMachine implements IMachine, IDisplay, IMemory {
  ...
  public writeRange(address: number, data: Uint8Array | number[]) {
    this.ram.set(data, address)
  }
  ...
  private read_C01x(address: number, cycleCount: number): number {
    switch (address & 0xf) {
      ...
      case 0x9:  // VERTBLANK
        // *** should update(cycleCount) be done first?
        const inVBlank = this.displayClock.inVBlank
  ...
  public setJoystickValues(joystick: Joystick) {
...
class Disk2Card implements IDevice {
